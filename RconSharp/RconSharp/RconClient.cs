    ...
    public class RconClient
    {
        ...
        private RconClient(IChannel channel)
        {
            Channel = channel ??
                      throw new NullReferenceException(
                          "channel parameter must be an instance of a class implementing INetworkSocket interface");
            Encoding = Encoding.UTF8;
        }

        public event Action ConnectionClosed;

        public static RconClient Create(string host, int port)
        {
            return new RconClient(new SocketChannel(host, port));
        }

        public static RconClient Create(IChannel channel)
        {
            return new RconClient(channel);
        }
        ...
        private async Task WriteToPipeAsync(PipeWriter writer)
        {
            while (true)
            {
                ...
                {
                    Debug.WriteLine($"[{nameof(WriteToPipeAsync)}] Exception: {ex.Message}");
                    break;
                }
            ...
            while (true)
            {
                ...
                if (buffer.Length >= packetSize + 4)
                {
                    ...
                    if (!rconPacket.IsDummy)
                    {
                        ...
                        if (currentOperation.OriginalRequest.Type == PacketType.Auth &&
                            rconPacket.Type == PacketType.Response)
                        {
                        ...
                        else
                        {
                            ...
                            if (rconPacket.Id == -1)
                            {
                                currentOperation.TaskCompletionSource.SetException(
        ...
        private class Operation(RconPacket originalRequest, bool isMultiPacketResponse)
        {
            ...
            public void Add(RconPacket packet)
            {
